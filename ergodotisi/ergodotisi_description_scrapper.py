...
def scrape_job_data(job_id, url, loc, file_name):
    ...
    with dataframe_lock:
        ...
        df.to_csv(f"ergodotisi\\job_descriptions\\{file_name}", mode='a', encoding='utf-8', index=False, header=False)
...
def run(url_list):
    ...
    header_df.to_csv(f"ergodotisi\\job_descriptions\\{file_name}", index=False, header=False)
